...
const SportsAndFitnessClubsPage = () => {
  const [sportsClubs, setSportsClubs] = useState([]); // State to store sports clubs
  const navigate = useNavigate();

  useEffect(() => {
    // Fetch sports clubs from the backend
    axios
      .get("http://localhost:5000/Back/clubs/clubs/Sports") // Update endpoint for sports clubs
      .then((res) => {
        setSportsClubs(res.data);
      })
      .catch((e) => {
        console.error("Error fetching sports clubs:", e);
      });
  }, []);
      ...
      <div className="container mx-auto p-8">
        ...
        {sportsClubs.length === 0 && (
          <div className="text-center text-gray-500 mt-12">
            No sports clubs available at the moment.
          </div>
        )}
      </div>
    </div>
